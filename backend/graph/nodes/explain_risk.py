...
_INTROS: dict[str, str] = {
    ...
    "gu": "અહીં Tarang એ જોખમ સ્કોરની ગણતરી કેવી રીતે કરી તેની વિગત છે:",
    "bn": "Tarang কীভাবে ঝুঁকির স্কোর গণনা করেছে তার বিবরণ এখানে দেওয়া হলো:",
    "te": "Tarang ప్రమాద స్కోరును ఎలా లెక్కించిందో ఇక్కడ వివరాలు ఉన్నాయి:",
    "ml": "Tarang എങ്ങനെയാണ് അപകടസാധ്യത സ്കോർ കണക്കാക്കിയതെന്ന് താഴെ നൽകുന്നു:",
    "mr": "Tarang ने धोका स्कोअर कसा मोजला याचा तपशील येथे आहे:",
    "od": "Tarang କିପରି ବିପଦ ସ୍କୋର ଗଣନା କରିଛି ତାର ବିବରଣୀ ଏଠାରେ ଦିଆଗଲା:",
}
...
_TOTALS: dict[str, str] = {
    ...
    "gu": "\n**કુલ: {total:.1f}/100 → {label}**\n\n{recommendation}",
    "bn": "\n**মোট: {total:.1f}/100 → {label}**\n\n{recommendation}",
    "te": "\n**మొత్తం: {total:.1f}/100 → {label}**\n\n{recommendation}",
    "ml": "\n**ആകെ: {total:.1f}/100 → {label}**\n\n{recommendation}",
    "mr": "\n**एकूण: {total:.1f}/100 → {label}**\n\n{recommendation}",
    "od": "\n**ମୋଟ: {total:.1f}/100 → {label}**\n\n{recommendation}",
}

_NO_RISK_DATA: dict[str, str] = {
    "en": (
        "No previous risk assessment is available to explain. "
        "Please ask a safety query first (e.g. 'Is it safe to fish near Kochi tomorrow?') "
        "and then ask for an explanation."
    ),
    "hi": (
        "समझाने के लिए कोई पिछला जोखिम आकलन उपलब्ध नहीं है। "
        "पहले एक सुरक्षा प्रश्न पूछें (उदा. 'क्या कल कोच्चि के पास मछली पकड़ना सुरक्षित है?') "
        "फिर स्पष्टीकरण मांगें।"
    ),
    "ta": (
        "விளக்க முந்தைய ஆபத்து மதிப்பீடு எதுவும் இல்லை. "
        "முதலில் ஒரு பாதுகாப்பு கேள்வியை கேளுங்கள் (எ.கா. 'நாளை கொச்சி அருகில் மீன்பிடிக்க பாதுகாப்பானதா?') "
        "பின்னர் விளக்கம் கேளுங்கள்."
    ),
    "gu": (
        "સમજાવવા માટે કોઈ પાછલું જોખમ આકલન ઉપલબ્ધ નથી. "
        "કૃપા કરીને પહેલાં સુરક્ષા પ્રશ્ન પૂછો (દા.ત. 'શું કાલે વેરાવળ પાસે જવું સલામત છે?') "
        "અને પછી ખુલાસો માંગો."
    ),
    "bn": (
        "ব্যাখ্যা করার জন্য কোনো পূর্ববর্তী ঝুঁকি মূল্যায়ন উপলব্ধ নেই। "
        "প্রথমে একটি নিরাপত্তা সংক্রান্ত প্রশ্ন জিজ্ঞাসা করুন (যেমন 'দীঘায় কাল মাছ ধরা কি নিরাপদ?') "
        "এবং তারপর ব্যাখ্যা চান।"
    ),
    "te": (
        "వివరించడానికి మునుపటి ప్రమాద అంచనా లేదు. "
        "దయచేసి ముందుగా భద్రతా ప్రశ్న అడగండి (ఉదా. 'రేపు విశాఖపట్నం దగ్గర చేపల వేట సురక్షితమేనా?')."
    ),
    "ml": (
        "വിശദീകരിക്കാൻ മുൻപത്തെ അപകടസാധ്യത വിലയിരുത്തലുകൾ ലഭ്യമല്ല. "
        "ദയവായി ആദ്യം ഒരു സുരക്ഷാ ചോദ്യം ചോദിക്കുക (ഉദാ. 'നാളെ കൊച്ചിയിൽ മീൻപിടിക്കാൻ പോകുന്നത് സുരക്ഷിതമാണോ?')."
    ),
    "mr": (
        "स्पष्टीकरणासाठी मागील कोणतेही धोका मूल्यांकन उपलब्ध नाही. "
        "कृपया आधी सुरक्षा प्रश्न विचारा (उदा. 'उद्या रत्नागिरी जवळ जाणे सुरक्षित आहे का?') आणि नंतर स्पष्टीकरण मागा."
    ),
    "od": (
        "ବୁଝାଇବା ପାଇଁ ପୂର୍ବର କୌଣସି ବିପଦ ଆକଳନ ଉପଲବ୍ଧ ନାହିଁ। "
        "ଦୟାକରି ପ୍ରଥମେ ଏକ ସୁରକ୍ଷା ପ୍ରଶ୍ନ ପଚାରନ୍ତୁ।"
    ),
}
...
_DISCLAIMER: dict[str, str] = {
    ...
    "gu": (
        "\n⚠️ **અસ્વીકરણ**: આ એક નિર્ણય સહાય આકલન છે, સત્તાવાર સુરક્ષા મંજૂરી નથી."
    ),
    "bn": (
        "\n⚠️ **সতর্কবার্তা**: এটি একটি সিদ্ধান্ত সহায়তা ব্যবস্থা, কোনো সরকারি অনুমোদন নয়।"
    ),
    "te": (
        "\n⚠️ **గమనిକ**: ఇది సహాయక నిర్ణయ సాధనం మాత్రమే, అధికారిక అనుమతి కాదు."
    ),
    "ml": (
        "\n⚠️ **ശ്രദ്ധിക്കുക**: ഇത് ഒരു തീരുമാന സഹായ സംവിധാനമാണ്, ഔദ്യോഗിക അനുമതിയല്ല."
    ),
    "mr": (
        "\n⚠️ **अस्वीकरण**: हे निर्णय-सहाय्य मूल्यांकन आहे, अधिकृत सुरक्षा मंजुरी नाही."
    ),
    "od": (
        "\n⚠️ **ଦାୟିତ୍ୱହୀନତା**: ଏହା ଏକ ନିଷ୍ପତ୍ତି ସହାୟତା ମୂଲ୍ୟାଙ୍କନ, ସରକାରୀ ଅନୁମତି ନୁହେଁ।"
    ),
}
...
def explain_risk(state: ORCAState) -> dict:
    ...
    if components:
        summary_title = {
            ...
            "gu": "તકનીકી જોખમ વિગતો જુઓ",
            "bn": "প্রযুক্তিগত ঝুঁকির বিবরণ দেখুন",
            "te": "సాంకేతిక ప్రమాద వివరాలను చూడండి",
            "ml": "സാങ്കേതിക വിവരങ്ങൾ കാണുക",
            "mr": "तांत्रिक धोका तपशील पहा",
            "od": "ବୈଷୟିକ ବିପଦ ବିବରଣୀ ଦେଖନ୍ତୁ",
